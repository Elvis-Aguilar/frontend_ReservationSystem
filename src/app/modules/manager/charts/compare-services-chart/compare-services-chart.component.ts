...
@Component({
  selector: 'app-compare-services-chart',
  standalone: true,
  imports: [],
  templateUrl: './compare-services-chart.component.html',
  styleUrls: ['./compare-services-chart.component.scss']
})
export class CompareServicesChartComponent {
  ...
  calcularResevacionesPorServicio(filterFechas: [string, string]): ServicesCount[] {
    ...
    for (const [serviceId, count] of Object.entries(serviceCount)) {
      ...
      const servi = this.services.find(ser => ser.id === idService)?.name
      const tmp = {
        name: servi || '',
        count: count
      }
